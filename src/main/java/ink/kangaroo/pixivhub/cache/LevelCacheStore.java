...
import ink.kangaroo.pixivhub.utils.StringUtils;
...
public class LevelCacheStore extends AbstractStringCacheStore {
    ...
    @Autowired
    private TribeProperties tribeProperties;
    ...
    public void init() {
        ...
        try {
            //work path
            File folder = new File(tribeProperties.getWorkDir() + ".leveldb");
...
//            log.info("LEVEL_DB bytes - > {}",valueJson);
//            log.info("isEmpty(valueJson) - > {}",StringUtils.isEmpty(valueJson));
//            log.info("jsonToCacheWrapper(valueJson) - > {}",jsonToCacheWrapper(valueJson));
    ...
    Boolean putInternalIfAbsent(String key, CacheWrapper<String> cacheWrapper) {
        ...
        LEVEL_DB.put(
                stringToBytes(key),
                stringToBytes(JSON.toJSONString(cacheWrapper))
        );
